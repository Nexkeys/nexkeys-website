 ...
 *
 * The internship is now open globally, so the form can no longer assume a
 * Nigerian `+234` number. Applicants pick their country and the dial code
 * follows from it.
 *
 * `code` is ISO 3166-1 alpha-2 and is what gets STORED — names and dial codes
 * change over time, two-letter codes effectively do not.
 */
...
/**
 * Flag emoji derived from the ISO code by mapping A–Z onto the Unicode
 * regional-indicator block. Avoids shipping 200 hardcoded emoji, and stays
...
/** Shown at the top of the list — the agency's main markets. */
export const PRIORITY_COUNTRIES = ['NG', 'GH', 'KE', 'ZA', 'GB', 'US', 'CA', 'AE'];
...
export const ORDERED_COUNTRIES: Country[] = [
  ...PRIORITY_COUNTRIES.map((code) => COUNTRY_BY_CODE[code]).filter(Boolean),
  ...COUNTRIES.filter((c) => !PRIORITY_COUNTRIES.includes(c.code)),
];

/**
 * Best-effort guess from the browser's timezone, so most applicants never have
 * to open the dropdown. Falls back to the default on anything unexpected.
 */
export function guessCountry(): string {
  try {
    ...
    const match = COUNTRIES.find(
      (c) => c.name.toLowerCase().replace(/\s+/g, '_') === region.toLowerCase()
    );
    if (match) return match.code;

    const locale = navigator.language ?? '';
    const fromLocale = locale.split('-')[1]?.toUpperCase();
    if (fromLocale && COUNTRY_BY_CODE[fromLocale]) return fromLocale;
  } catch {
    /* ignore */
  }
...
/** Digits only, no leading zeros — what gets combined with the dial code. */
export function normaliseSubscriber(input: string): string {
  return input.replace(/\D/g, '').replace(/^0+/, '');
}

/** Full E.164-style number, e.g. +2348012345678. */
export function fullPhoneNumber(dial: string, subscriber: string): string {
  return `+${dial}${normaliseSubscriber(subscriber)}`;
}
